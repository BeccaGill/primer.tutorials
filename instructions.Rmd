...
### Using files in tutorials

Dealing with external files in tutorials is confusing! 

When we have students read in files, we actually create the file in a directory called `www`, which lives in the `inst` directory. This is the file the students actually read in during the tutorial (not the one they create in revious questions). 

Recall each exercise acts as its own R session. We need to get the example file into the working area for each question. To do so, we create a setup chunk under each exercise that uses the function `file.copy()`.

Here is an example of an .rds file,"test_1.rds", being copied into the exercise workspace.
````
```{r ex-2-rds-setup}
